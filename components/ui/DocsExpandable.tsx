...
export default function DocsExpandable({ items, initialVisible = 5 }: { items: string[]; initialVisible?: number }) {
            ...
            <ul className="space-y-2">
                {visible.map((it, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                        <span className="flex-none w-7 h-7 rounded-full bg-[var(--primary)] text-[var(--secondary)] flex items-center justify-center mt-1"> 
                ...
                {items.length > initialVisible && (
                    <li className="flex justify-center">
                        ...
                        >
                            {expanded ? 'Moins' : `Plus (${items.length - initialVisible})`}
                        </button>
                    </li>
                )}
            </ul>
